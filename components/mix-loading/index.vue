<template>
	...
	<view class="mix-loading-content">
		...
		<!-- 别的页面引入使用 refTriggered当前下拉的状态 -->
		<!-- <scroll-view scroll-y enable-back-to-top refresher-enabled @refresherrefresh='handleRefresh' @scrolltolower='handleMore'
		 style="height: 100%;" :refresher-triggered='triggered'> -->
			 <!-- handleRefresh上滑触发的加载事件 handleMore下拉触发的加载事件 下拉加载的状态triggered true三个点展示false隐藏
			 加载的状态 status 1有数据加载 2正在加载 3没有数据加载了 -->
			 <!-- 加载动画 -->
			 <!-- <view :class="['loading', refTriggered ? '' : 'not']">
				<loading :top='30' />
			 </view> -->
			 <!-- 加载状态 -->
			 <!-- <view class="bottom-loading h-30 flex mt10 fons-12">
